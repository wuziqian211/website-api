...
| `cookie` | 获取信息时是否带 Cookie。如果本参数的值为 `true`，则强制带 Cookie 获取信息；如果值为 `false`，则强制不带 Cookie 获取信息；否则先尝试不带 Cookie 获取信息，如果失败，再带 Cookie 获取信息。本参数的值不区分大小写。<br />**示例**：[BV16s411f7\x2](https://api.yumeharu.top/api/getvideo?vid=BV16s411f7x2&cookie=true) |

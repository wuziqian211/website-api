...
export default async (req, res) => {
  ...
  try {
    ...
    if (/^\d+$/.test(req.query.mid)) { // 判断UID是否是非负整数
      ...
      ${json.data.official.type === 0 || json.data.official.type === 1 ? `<img class="official-icon" alt="" title="${json.data.official.type === 0 ? 'UP 主' : '机构'}认证" src="/assets/official_${json.data.official.type === 0 ? 'personal' : 'organization'}.png" /><strong>bilibili ${json.data.official.type === 0 ? 'UP 主' : '机构'}认证：</strong>${utils.encodeHTML(json.data.official.title)}<br />

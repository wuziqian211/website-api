  ...
  try {
    ...
    if (/^\d+$/.test(req.query.mid) && parseInt(req.query.mid) > 0) { // 判断 UID 是否是正整数
      ...
      if (cjson.code === 0) {
        ...
        const ujson = await (await fetch(`https://api.bilibili.com/x/space/wbi/acc/info?${await encodeWbi({ mid: req.query.mid }, await getWbiKeys())}`, { headers })).json(); // （备用）获取多用户信息 https://api.vc.bilibili.com/account/v1/user/cards?uids=xxx,xxx,……（最多50个）
        if (ujson.code === 0) {

...
export default async (req, res) => {
  ...
  try {
    ...
    if (/^\d+$/.test(req.query.mid)) { // 判断UID是否是非负整数
      ...
      ${json.data.sys_notice?.content ? `<${json.data.sys_notice.url ? `a class="no-underline" target="_blank" rel="noopener external nofollow noreferrer" href="${json.data.sys_notice.url}"` : 'span'} style="background: ${json.data.sys_notice.bg_color || 'none'}; color: ${json.data.sys_notice.text_color || 'inherit'};">${json.data.sys_notice.icon ? `<img class="unotice-icon" alt="" src="${utils.toHTTPS(json.data.sys_notice.icon)}" referrerpolicy="no-referrer" /> ` : ''}<strong>${json.data.sys_notice.content}</strong></${json.data.sys_notice.url ? 'a' : 'span'}><br />

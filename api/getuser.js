...
export default async (req, res) => {
  ...
  try {
    ...
    if (/^\d+$/.test(req.query.mid)) { // 判断UID是否是非负整数
      ...
      ` : ''}<strong>生日：</strong>${json.data[0].birthday ? getDate(json.data[0].birthday).slice(0, 10) : '保密'}<br />
    ...
    ` : '';
              if (req.query.type === 'info') { // 仅获取用户信息
                sendHTML({ title: `${utils.encodeHTML(json.data[0].name)} 的信息`, style: utils.renderExtraStyle('/assets/top-photo.png') + extraStyle, content, mid: req.query.mid });
              } else {
                const fjson = await (await fetch(`https://api.bilibili.com/x/relation/stat?vmid=${req.query.mid}`, { headers })).json();
                if (fjson.code === 0) {
                  sendHTML({ title: `${utils.encodeHTML(json.data[0].name)} 的信息及关注、粉丝数`, style: utils.renderExtraStyle('/assets/top-photo.png') + extraStyle, content: content + `<br />

                ...
                <div class="main-info-outer">
                  <div class="main-info-inner">
                    ...
                    <div class="detail">
                      ...
                      <br />
                      ${data.attestation.type ? `${[1, 2].includes(data.attestation.type) ? `<img class="official-icon icon-personal" alt="⚡" title="UP 主${data.attestation.type === 1 ? '专业' : ''}认证" /> <strong class="text-personal">bilibili UP 主${data.attestation.type === 1 ? '专业' : ''}认证` : data.attestation.type === 3 ? '<img class="official-icon icon-business" alt="⚡" title="机构认证" /> <strong class="text-business">bilibili 机构认证' : `<img class="official-icon" alt="⚡" title="${data.attestation.common_info.prefix} src="${utils.toHTTPS(data.attestation.icon)}" /"> <strong class="text-personal">${data.attestation.common_info.prefix}`}${data.attestation.common_info.title ? '：' : ''}</strong>${utils.encodeHTML(data.attestation.common_info.title)}${data.attestation.desc ? `<span class="description">（${utils.encodeHTML(data.attestation.desc)}）</span>` : ''}<br />` : ''}

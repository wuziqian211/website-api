...
import type { booleanNumber, numericString, SendHTMLData, InternalAPIResponse, APIResponse, UserCardData, UserInfoData, UserCardsData, UsersInfoItem, UsersInfoData, InternalAPIGetUserInfoData, InternalAPIGetUsersInfoData } from '../assets/types.d.ts';
...
export default {
  async fetch(req: Request): Promise<Response> {
    ...
    try {
      const sendHTML = (status: number, data: Omit<SendHTMLData, 'body'> & { content: string; mid?: string }): void => resolve(utils.sendHTML(session, status, { ...data, desc: '获取哔哩哔哩用户信息', body: `
        ${data.content}
        <form>
      ...
      if (requestMid && /^\d+$/.test(requestMid) && BigInt(requestMid) > 0) { // 客户端提供的 UID 只有一个且有效
          ...
          case 1: { // 回复 HTML
            switch (json.code) {
                ...
                const content = `
                  <div class="main-info">
                    ...
                    <div class="detail">
                      ...
                      ${data.vip.status ? data.vip.label.use_img_label && (data.vip.label.img_label_uri_hans || data.vip.label.img_label_uri_hans_static) ? `<a target="_blank" rel="noopener external nofollow noreferrer" href="https://account.bilibili.com/big"><img class="vip" alt="${utils.encodeHTML(data.vip.label.text)}" title="${utils.encodeHTML(data.vip.label.text)}（过期时间：${utils.getDate(data.vip.due_date)}）" src="${utils.toHTTPS(data.vip.label.img_label_uri_hans || data.vip.label.img_label_uri_hans_static)}" /></a>` : `<a class="vip" target="_blank" rel="noopener external nofollow noreferrer" href="https://account.bilibili.com/big" style="${data.vip.label.bg_color ? `background: ${utils.encodeHTML(data.vip.label.bg_color)};` : ''}${data.vip.label.text_color ? `color: ${utils.encodeHTML(data.vip.label.text_color)};` : ''}">${utils.encodeHTML(data.vip.label.text)}</a>` : ''}
                      ...
                      <br />
                      <span class="description">UID：${mid}</span>${data.fans_badge ? ' <span class="description">粉丝勋章</span>' : ''}${data.silence || data.control ? ` <span class="notice"><img class="notice-icon" alt="⚠️" /> 该账号封禁中${data.control ? '，且无法查看空间' : ''}</span>` : ''}
                      <br />
              ...
              case -799: // 请求过于频繁，请稍后再试（URL 非 Wbi）
                responseHeaders.set('Retry-After', '600');
        ...
        while (restUsers.length) {
          ...
          ujsonList.push(<Promise<APIResponse<UsersInfoData>>> utils.callAPI(session, 'https://api.vc.bilibili.com/account/v1/user/cards', { params: { uids: restUsers.splice(0, 50).join(',') }, withCookie: true }));
        }
        ...
        if (responseType === 1) { // 回复 HTML
              ...
              <div class="grid user-list">
                ...
                <div class="grid-item main-info image-background" id="user-${u.mid}" style="--background-image: url(${utils.toHTTPS(u.face)});">
                  ...
                  <div class="detail">
                    ...
                    ${u.vip.status ? u.vip.label.use_img_label && (u.vip.label.img_label_uri_hans || u.vip.label.img_label_uri_hans_static) ? `<a target="_blank" rel="noopener external nofollow noreferrer" href="https://account.bilibili.com/big"><img class="vip" alt="${utils.encodeHTML(u.vip.label.text)}" title="${utils.encodeHTML(u.vip.label.text)}（过期时间：${utils.getDate(+u.vip.due_date)}）" src="${utils.toHTTPS(u.vip.label.img_label_uri_hans || u.vip.label.img_label_uri_hans_static)}" /></a>` : `<a class="vip" target="_blank" rel="noopener external nofollow noreferrer" href="https://account.bilibili.com/big" style="${u.vip.label.bg_color ? `background: ${utils.encodeHTML(u.vip.label.bg_color)};` : ''}${u.vip.label.text_color ? `color: ${utils.encodeHTML(u.vip.label.text_color)};` : ''}">${utils.encodeHTML(u.vip.label.text)}</a>` : ''}
                    <br />
          ...
          if (Object.keys(data).length) {
            sendJSON(200, { code: 0, message: 'OK', data });
          } else {
        ...
        switch (responseType) {
          case 1: // 回复 HTML
            if (!requestMid) { // 没有设置 UID 参数
              ...
              responseHeaders.set('Cache-Control', 's-maxage=86400, stale-while-revalidate');

    ...
    if (type === 1) { // 编号为AV号或BV号
        ...
        if (accept === 1) { // 客户端想要获取类型为“文档”的数据，返回HTML
          switch (json.code) {
            case 0:
              ...
              const content = `
                <a class="title" target="_blank" rel="noopener external nofollow noreferrer" href="https://www.bilibili.com/video/${vid}">
                  <img class="vpic" alt title="${utils.encodeHTML(json.data.title)}" src="${utils.toHTTPS(json.data.pic)}" referrerpolicy="no-referrer" />
                  ${utils.encodeHTML(json.data.title)}
                </a>
                （av${json.data.aid}，${utils.encodeHTML(json.data.bvid)}）
                ${json.data.forward ? `（已与 <a href="?vid=${utils.toBV(json.data.forward)}">${utils.toBV(json.data.forward)}</a> 撞车）` : ''}
                <br />
                ...
                <strong>合作成员：</strong><br />${json.data.staff.map(u => `<a class="title" target="_blank" rel="noopener external nofollow noreferrer" href="https://space.bilibili.com/${u.mid}"><img class="face" alt title="${utils.encodeHTML(u.name)}" src="${utils.toHTTPS(u.face)}" referrerpolicy="no-referrer" /> ${utils.encodeHTML(u.name)}</a>（<strong>粉丝数：</strong>${utils.getNumber(u.follower)}） ${utils.encodeHTML(u.title)}`).join('<br />')}` : `
      ...
      if (accept === 1) { // 客户端想要获取类型为“文档”的数据，返回HTML
        switch (json.code) {
          case 0:
            const content = `
              <a class="title" target="_blank" rel="noopener external nofollow noreferrer" href="https://www.bilibili.com/bangumi/media/md${vid}">
                <img class="spic" alt title="${utils.encodeHTML(json.result.media.title)}" src="${utils.toHTTPS(json.result.media.cover)}" referrerpolicy="no-referrer" />
                ${utils.encodeHTML(json.result.media.title)}
              </a>
              <br />
              ...
              ${json.result.media.new_ep?.id ? `<br /><strong>最新一话：</strong><a href="?vid=ep${json.result.media.new_ep.id}">${utils.encodeHTML(json.result.media.new_ep.index)}</a>` : ''}
              ${json.result.media.season_id ? `<br /><a href="?vid=ss${json.result.media.season_id}">点击此处查看更多信息</a>` : ''}`;
            res.status(200);
      ...
      if (req.query.type === 'data') { // 获取剧集中某一集的视频数据
        ...
        if (accept === 1) { // 客户端想要获取类型为“文档”的数据，返回HTML
          switch (json.code) {
            case 0:
              const content = `
                <a class="title" target="_blank" rel="noopener external nofollow noreferrer" href="https://www.bilibili.com/bangumi/play/${type === 3 ? 'ss' : 'ep'}${vid}">
                  <img class="spic" alt title="${utils.encodeHTML(json.result.title)}" src="${utils.toHTTPS(json.result.cover)}" referrerpolicy="no-referrer" />
                  ${utils.encodeHTML(json.result.title)}
                </a>
                <br />

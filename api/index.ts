    ...
    try {
      ...
      if (session.responseType === 1) {
        ...
        return utils.sendHTML(session, 200, { title: '欢迎来到 API 页面', newStyle: true, body: `
          ...
          <p>本站提供以下公开的 API：</p>
          <div class="grid">
            <div class="grid-item"><p class="grid-title">获取哔哩哔哩用户信息</p><p>本 API 可以获取指定 B 站用户的信息</p><a class="grid-link" href="/api/getuser"></a></div>
            <div class="grid-item"><p class="grid-title">获取哔哩哔哩视频 / 剧集 / 番剧信息及数据</p><p>本 API 可以获取指定 B 站视频、剧集、番剧的信息及数据</p><a class="grid-link" href="/api/getvideo"></a></div>
          </div>

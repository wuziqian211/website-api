      ...
      case 'friends':
        ...
        const get = async users => {
          if (users.length > 0) {
            ...
            let html = '';
            info.sort(() => 0.5 - Math.random()).forEach(u => html += `<div class="link-grid-container"><img class="link-grid-image" src="${encodeHTML(u.face)}" referrerpolicy="no-referrer" /><p>${encodeHTML(u.name)}</p><p>${encodeHTML(u.sign)}</p><a target="_blank" rel="noopener external nofollow noreferrer" href="https://space.bilibili.com/${u.mid}"></a></div>`);

        ...
        if (json.code == 0) {
          fetch(`https://api.bilibili.com/x/relation/stat?vmid=${req.query.mid}`).then(resp => resp.json()).then(fjson => {
            if (fjson.code == 0) {
              ...
              sendHTML({code: 200, title: `${json.data.name} 的用户信息`, face: ')', content: `昵称：${json.data.name}<br />头像：<br /><img alt="${json.data.name} 的头像" src="${json.data.face}" referrerpolicy="no-referrer" />`, mid: req.query.mid, tips: 'OK'});
            }
          });

  ...
  if (/^[0-9]+$/.test(req.query.mid)) { // 判断UID是否是非负整数
      ...
      fetch(`https://api.bilibili.com/x/space/acc/info?mid=${req.query.mid}`).then(resp => resp.json()).then(json => {
        if ((req.headers.accept && req.headers.accept.indexOf('html') != -1) || req.headers['x-pjax'] == 'true') { // 客户端提供的接受类型有HTML，或者是Pjax发出的请求，返回HTML
          ...
          if (json.code == 0) {
            ...
            fetch(`${process.env.URL}/res/noface.jpg`).then(img => img.buffer()).then(buffer => res.status(404).setHeader('Content-Type', 'image/jpg').setHeader('Content-Disposition', 'inline;filename=%E7%94%A8%E6%88%B7%E4%B8%8D%E5%AD%98%E5%9C%A8.jpg').send(buffer));
          }
    ...
    if ((req.headers.accept && req.headers.accept.indexOf('html') != -1) || req.headers['x-pjax'] == 'true') { // 客户端提供的接受类型有HTML，或者是Pjax发出的请求，返回HTML
      ...
      if (!req.query.mid || req.query.mid == '') { // 没有设置UID参数，返回随机头像
        ...
        fetch(`${process.env.URL}/res/${faces[Math.floor(Math.random() * 11)]}.jpg`).then(img => img.buffer()).then(buffer => res.status(200).setHeader('Content-Type', 'image/jpg').send(buffer));
      } else { // 设置了UID参数但无效，返回默认头像
        fetch(`${process.env.URL}/res/noface.jpg`).then(img => img.buffer()).then(buffer => res.status(404).setHeader('Content-Type', 'image/jpg').send(buffer));      
      }
    } else { // 接受类型既不含HTML，也不含图片，返回json

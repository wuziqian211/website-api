    ...
    fetch(`https://api.bilibili.com/x/space/acc/info?mid=${req.query.mid}`).then(resp => resp.json()).then(json => {
      if (req.headers.accept && req.headers.accept.indexOf('html') != -1) {
        if (json.code == 0) {
          ...
          var d = {code: 404, title: '用户不存在', face: '(', content: `UID${req.query.mid} 对应的用户不存在！QAQ`, tips: 'NOT_FOUND'};
        } else {
          var d = {code: 400, title: '获取用户信息失败', face: '(', content: `获取 UID${req.query.mid} 的信息失败，请稍后重试awa`, tips: 'BAD_REQUEST'};
        }
...
img {
  ...
  <body>
    ...
    <p>${d.content}</p>
    <form action="/getbili.js" method="GET">请输入要获取用户信息的 UID：<input type="text" name="mid" value="${q.query.mid}"><input type="submit" value="获取"></form>
